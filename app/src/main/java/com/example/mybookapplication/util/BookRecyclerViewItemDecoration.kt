...
class RecyclerViewItemDecoration(
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        val dividerLeft: Int = 200

        // right margin for the divider with
        // reference to the parent width
        val dividerRight: Int = parent.width - 32

        // this loop creates the top and bottom divider for each items in the RV
        // as each items are different
        for (i in 0 until parent.childCount) {

            // this condition is because the last and the first items in the RV have
